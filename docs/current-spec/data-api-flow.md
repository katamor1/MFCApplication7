...
`BuildMaintenanceStatusModel()` は `catalog.CriticalKeys()` と `UpdateSnapshot::criticalValues` を同じ順序で対応付け、重要情報ごとの保守ステータス行を作ります。行は dataId、カタログ名、表示値、エラー、stale、異常判定、操作可を持ちます。

異常判定はV1の暫定仕様です。値不足、空文字、`stale=true`、非OKエラー、または表示文字列に `異常` を含む場合は異常扱いになります。異常行だけ `BuildMaintenanceFunctionActions()` のF1詳細が有効になり、`BuildMaintenanceDetailModel()` が読み取り専用詳細行を生成します。
