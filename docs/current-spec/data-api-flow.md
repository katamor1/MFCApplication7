...
`MockBackendBridge` は検証用に `MockLoadProfile` と `MockLatencyOptions` を持ちます。

| 設定 | 現行動作 |
|---|---|
| `MockLoadProfile::Default` | 従来互換の軽量データ。コンテナなしや品目数は規則生成値に従う。 |
| `MockLoadProfile::MaxLoad` | 100コンテナすべてを有効扱いにし、各コンテナの品目数 `2003` を `10` として返す。`BuildScheduleGrid()` は1000品目相当を生成できる。 |
| `criticalReadDelayMs` | `1000-1019` のReadごとに指定ミリ秒sleepする。 |
| `normalReadDelayMs` | 通常/スケジュール系Readごとに指定ミリ秒sleepする。 |
| `historyReadDelayMs` | `4000` の履歴Readごとに指定ミリ秒sleepする。 |
| `writeDelayMs` | Write呼び出しごとに指定ミリ秒sleepする。 |

これらはインプロセスmockの検証機能です。COM mode では `ComBackendBridge` が選ばれるため、モック負荷プロファイルと遅延注入は適用されません。
...
`BuildMaintenanceStatusModel()` は `catalog.CriticalKeys()` と `UpdateSnapshot::criticalValues` を同じ順序で対応付け、重要情報ごとの保守ステータス行を作ります。行は dataId、カタログ名、表示値、エラー、stale、異常判定、操作可、判断支援を持ちます。

異常判定はV1の暫定仕様です。値不足、空文字、`stale=true`、非OKエラー、または表示文字列に `異常` を含む場合は異常扱いになります。`BuildMaintenanceSupportHint()` は異常原因を `ReadError`、`Stale`、`MissingValue`、`AbnormalText`、`Unknown` に分類し、確認優先度、推奨確認、管理者メモを作ります。正常行は `None` で、追加確認不要の表示になります。

異常行だけ `BuildMaintenanceFunctionActions()` のF1詳細が有効になり、`BuildMaintenanceDetailModel()` が読み取り専用詳細行を生成します。詳細行は従来のデータID、名称、値、状態、エラー、stale、操作可に加え、原因分類、確認優先度、推奨確認、管理者メモを含みます。

互換用の `BuildMaintenanceGrid()` は残っていますが、現行UIのMaintenance画面は `BuildMaintenanceStatusGrid()` を使います。正式な保守対象コンテナ、復旧操作Write、管理者権限制御は未実装です。
