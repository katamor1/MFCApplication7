...
- ステーション画面の物理配置風表示、3列コンテナ一覧、詳細ダイアログ本体、左ナビオーバーレイ、F1-F8 キーボード入力変換などは部分実装であり、正式仕様に依存する残課題がある。
- カスタムグリッドは `CellKind` に応じた編集UI基盤を持ち、Schedule 画面の品目名、出庫開始予定、出庫終了予定、出庫順序を既存のWrite IDへ接続している。
