...
| U-009 | 部分実装 | スケジュールの有効行で、順序繰り上げ、出庫予定の追加・削除をファンクションキーから実行する。 | 順序変更は `2103`、追加は `2104`、削除は `2105` のWriteキュー経由で実行でき、Mock/COM MockでRead反映を自己診断する。 | 順序繰り上げ操作、複数行への再採番、追加/削除失敗時の業務復旧表示が未実装。 | `src/Core/FunctionBarModel.cpp` の `BuildScheduleFunctionActions`; `src/App/MainDialog.cpp` の `AddScheduleItem`/`DeleteScheduleItem`; `tests/CoreTests/main.cpp` の schedule mutation tests | P2 | 順序繰り上げと再採番ルール、失敗時の復旧UIを設計する。 |
